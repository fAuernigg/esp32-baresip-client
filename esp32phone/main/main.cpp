...
void callback(char* topic, byte* msg, unsigned int length)
{
  ...
  if (cmd == "cmd") {
    ...
    ESP_LOGI(TAG, "Cmd Message received: %s", message.c_str());
      ...
      ESP_LOGI(TAG, "mqtt reboot cmd sent, Rebooting ...");
      delay(500);
      ESP.restart();
...
void mqttSendPing()
{
  ...
  if ( gLastMsg ==-1 || (now - gLastMsg) > MQTT_SENDVERSION_INTERVAL) {
      gLastMsg = now;
      String v = String(mqtt_id + "/version");
      String l = String(mqtt_id + "/localip");
      String ip = WiFi.localIP().toString();
      ESP_LOGI(TAG,"mqtt publish version...");
      mqttClient.publish(v.c_str(), VERSION "-pre" BUILDNR );
      mqttClient.publish(l.c_str(), ip.c_str());
  }
}
